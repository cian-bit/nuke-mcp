...
def _handle_audit_acescct_consistency(params: dict) -> dict:
    """Delegate to the C2 colour-module handler if installed.

    Returns the C2 handler's payload verbatim. When C2 hasn't shipped
    its addon-side handler the dispatcher would never route here in
    the first place; this stub is the safety net so that a partial
    deploy (server-side audit module + no addon update) returns a
    diagnosable degraded shape instead of an "unknown command" error.
    """
    handler = HANDLERS.get("audit_acescct_consistency_color")
    if handler is None or handler is _handle_audit_acescct_consistency:
        return {
            "findings": [],
